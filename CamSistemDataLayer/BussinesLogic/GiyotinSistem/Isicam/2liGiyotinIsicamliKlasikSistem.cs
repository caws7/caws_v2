...
{
    public static class _2liGiyotinIsicamliKlasikSistem
    {
            ...
            foreach (Profil item in profilList)
            {
                switch (item.ProfilKodu)
                {
                    ...
                    case "G-106-1"://dikey baza üst cam
                        item.KesimOlcusu = Convert.ToInt32((((double)boy - 201.69) / 2) + 2.197) + 14 + 20;
                        item.KesimAdet = adet * 2;
                        item.ToplamAgirlik = profilToplamAgirlikHesaplama(item.KesimOlcusu, item.KesimAdet, (int)item.BirimAgirlik);
                        break;
                    case "G-106-3"://sabit cam
                        item.KesimOlcusu = Convert.ToInt32((((double)boy - 201.69) / 2) - 2.197 + (22+20));
                        item.KesimAdet = adet * 2;
                        item.ToplamAgirlik = profilToplamAgirlikHesaplama(item.KesimOlcusu, item.KesimAdet, (int)item.BirimAgirlik);
                        break;
                    case "G-107":
                    ...
                    case "G-109":
                        ...
                        break;
                    case "G-110-1":
                        item.KesimOlcusu = Convert.ToInt32(((((double)boy - 201.69) / 2) + 2.197) - 13);
                        item.KesimAdet = adet * 2;
                        item.ToplamAgirlik = profilToplamAgirlikHesaplama(item.KesimOlcusu, item.KesimAdet, (int)item.BirimAgirlik);
                        break;
                    case "G-110-2":
