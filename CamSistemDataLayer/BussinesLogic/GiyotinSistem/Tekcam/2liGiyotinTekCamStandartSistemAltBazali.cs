...
{
    public static class _2liGiyotinTekCamStandartSistemAltBazali
    {
            ...
            foreach (Profil item in profilList)
            {
                switch (item.ProfilKodu)
                {
                    ...
                    case "G-120":
                        ...
                        break;
                    case "G-121":
                        item.KesimOlcusu = Convert.ToInt32((((double)(boy - 187.144) / 2) - 1.361 - 5) + 13.38 + 11.38);
                        item.KesimAdet = adet * 2;
                        item.ToplamAgirlik = profilToplamAgirlikHesaplama(item.KesimOlcusu, item.KesimAdet, (int)item.BirimAgirlik);
                        break;
                    ...
                    case "G-125":
                        ...
                        break;
                    case "G-126":
                        item.KesimOlcusu = Convert.ToInt32(((((double)(boy - 187.144) / 2) - 1.361 - 5) - 16.594) + 15.277);
                        item.KesimAdet = adet * 2;
                        item.ToplamAgirlik = profilToplamAgirlikHesaplama(item.KesimOlcusu, item.KesimAdet, (int)item.BirimAgirlik);
                        break;
                    case "SB-101":
        ...
        public static List<CamBilgileri> CamYukseklikHesapla(int boy, int en, int adet)
        {
            ...
            camModel.Yukseklik = Convert.ToInt32((double)(boy - 187.144) / 2 - 1.361 - 5);
